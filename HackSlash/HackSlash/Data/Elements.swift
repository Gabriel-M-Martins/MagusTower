...
enum Elements {
    ...
    //    func thunderDebuff(_ attr: any Attributes & Status, for time: Double){
    //        if !attr.isBuffed{
    //            let emitter = SKEmitterNode(fileNamed: "EletricAura")!
    //            emitter.zPosition = -3
    //            attr.sprite.addChild(emitter)
    ...
    func fireDebuff(_ attr: any Attributes & Status, for time: Double){
        if !attr.isBuffed{
            var attrCopy = attr
            attrCopy.isBuffed = true
            let emitter = SKEmitterNode(fileNamed: "FireAura")!
            ...
            for i in 1...3{
                DispatchQueue.main.asyncAfter(deadline: .now() + Double(i)) {
                    attrCopy.attributes.health -= Constants.singleton.burnDamage
                    if i == 10{
                        emitter.removeFromParent()
                        attrCopy.isBuffed = false
                    }
    ...
    //    func earthDebuff(_ attr: any Attributes & Status, for time: Double){
    //        if !attr.isBuffed{
    //            let emitter = SKEmitterNode(fileNamed: "EarthAura")!
    //            emitter.zPosition = -3
    //            attr.sprite.addChild(emitter)
    ...
    func iceDebuff(_ attr: any Attributes & Status, for time: Double){
        if !attr.isBuffed{
            let emitter = SKEmitterNode(fileNamed: "IceAura")!
            emitter.zPosition = -3
            attr.sprite.addChild(emitter)
