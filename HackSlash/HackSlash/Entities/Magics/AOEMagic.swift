...
class AOEMagic: Projectile, DetectsCollision{
    ...
    func onTouch(touched: any Attributes & Status){
        ...
        if copySelf.attributes.weakness.contains(self.damage.element){
            copySelf.attributes.health -= self.damage.damage * 2
        }
        else if copySelf.attributes.resistence.contains(self.damage.element){
            copySelf.attributes.health -= self.damage.damage / 2
        }
        else{
